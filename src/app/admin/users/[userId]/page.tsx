...
function getOverallInfo(score: number) {
  if (score > 80)
    return {
      ...
      description:
        "你可能经常觉得累、卡、拧巴、不自由。但这不是你不行。更可能是你身上背了太多不属于你的声音。",
    };
  if (score > 60)
    return {
      label: "第四档：重度隐形内耗",
      seedling: "🌰 刚刚破土",
      description:
        "你的很多选择可能并不是从自己出发，而是被关系、权威、恐惧、羞耻、旧信念和家族脚本推着走。",
    };
  if (score > 40)
    return {
      label: "第三档：中度隐形内耗",
      seedling: "🌿 根系正在变深",
      description: "你已经知道很多问题和自己的内在模式有关。但真的遇到事情还是会被打回原形。",
    };
  if (score > 20)
    return {
      label: "第二档：轻中度隐形内耗",
      seedling: "🌱 正在扎根生长",
      description: "你已经有明显的自我意识，也开始知道自己不是只能活在别人的期待里。",
    };
  return {
    label: "第一档：轻度隐形内耗",
    seedling: "🌿 正在抽枝开花",
    description: "你的内在系统整体比较稳定，多数旧程序已经能被你觉察、识别和调整。",
  };
}
...
function getDimensionText(id: string, index: number): string {
  const texts: Record<string, { high: string; mid: string; low: string }> = {
    "self-worth": {
      high: "「证明程序」运行中。你内在有一个声音在说：你还不够好，你需要做得更多才值得被爱。",
      mid: "「摇摆模式」。你有时候觉得自己挺好的，有时候又会掉进'我不配'的漩涡。",
      low: "「内在稳定」。你对自己的价值有比较清晰的认知，不太容易被外界评价动摇。",
    },
    boundaries: {
      high: "「讨好程序」全速运行。你在关系中扮演的是'付出者''调和者''照顾者'，但很少是'自己'。",
      mid: "「有觉察但还做不到」。你已经意识到自己在关系中的讨好模式，但在真实场景中还是很难做到拒绝。",
      low: "「边界清晰」。你在关系中能比较好地保持自我，能温和但坚定地表达需求。",
    },
    decision: {
      high: "「外包模式」运行中。你的决策系统被外包了，内在有一个底层代码在说：'如果我选错了，那就是我的错'。",
      mid: "「半自主模式」。你在一些领域能自主决策，但在关键领域还是容易犹豫和依赖他人意见。",
      low: "「内在导航在线」。你有比较清晰的内在导航系统，能在收集信息后做出自主判断。",
    },
    emotion: {
      high: "「情绪海绵」模式。你是一块情绪海绵——周围人的情绪你全部吸收了，你的情绪系统一直在超负荷运转。",
      mid: "「部分渗透」。你能感知到自己有时候会被别人的情绪影响，但还能在一些时候找回自己的中心。",
      low: "「情绪主权在握」。你能比较好地分辨哪些情绪是自己的、哪些是别人的。",
    },
    action: {
      high: "「刹车模式」全开。你的行动通道被一个'内在刹车系统'卡住了，你不缺想法和能力，缺的是'允许自己开始'的许可。",
      mid: "「间歇启动」。你有时候可以突破阻力开始行动，但在关键时刻会卡住。",
      low: "「创造通道畅通」。你能比较快地将想法转化为行动，不追求完美主义，接受'边做边调整'。",
    },
    wealth: {
      high: "「漏斗模式」。你的财富容器有漏洞，不是你赚不到钱，是你的内在系统'承不住'钱。",
      mid: "「有限容器」。你能赚到一定水平的钱，但很难突破到下一个台阶。",
      low: "「财富容器扩容中」。你与金钱的关系比较健康，能自在地赚钱和花钱。",
    },
  };
  const data = texts[id];
  if (!data) return "";
...
export default async function AdminUserReportPage({ params }: PageProps) {
  ...
  const { user, assessment, membership, result } = data;
      ...
      <section className="paper-frame grid grid-rows-[56px_1fr]">
        ...
        {!result ? (
          <div className="grid place-items-center p-16 text-center">
            ...
            <a className="text-link mt-4" href="/admin">
              返回
            </a>
          </div>
        ) : (
            ...
            <aside className="grid min-h-0 grid-rows-[auto_auto_auto] gap-5 border-r border-[var(--line)] bg-paper/50 p-[clamp(20px,3vw,36px)] max-lg:border-b max-lg:border-r-0">
              ...
              <div className="self-center">
                <div className="flex items-end gap-3">
                  <strong className="text-[88px] font-normal leading-[.85]">
                    {Math.round(result.totalScore100)}
                  </strong>
            ...
            <section className="grid min-h-0 gap-0 overflow-auto p-[clamp(16px,2.4vw,28px)] max-lg:overflow-visible">
              <div className="mb-4 border-b border-[var(--line)] pb-4">
                ...
                <p className="mt-1 text-sm text-[var(--muted)]">
                  {dimensionRows.filter((r) => r.level === "high").length}个高位维度优先处理
                </p>
              </div>
                ...
                {dimensionRows.map((dimension) => (
                  <details
                    ...
                    className={`border border-[var(--line)] ${
                      dimension.level === "high" ? "border-clay/40 bg-[#f7ead8]/40" : "bg-soft/40"
                    }`}
                  >
                    <summary className="flex cursor-pointer items-center gap-3 p-4 list-none">
                      ...
                      <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full border border-[var(--line)] text-clay">
                        ▾
                      </span>
                    </summary>
                        ...
                        <div className="progress-track w-24">
                          <i
                            ...
                            style={{
                              ...
                              background:
                                dimension.index > 60 ? "var(--clay)" : dimension.index > 40 ? "#c8a05b" : "#6b9e78",
                            }}
                          />
                        </div>
