...
- `origin` 应该是 `git@github.com-chengta100:hellarychou-cell/-100.git`。
- 本机已经配置专用 SSH key：`~/.ssh/chengta100_github_deploy_ed25519`。
- `~/.ssh/config` 里应该有 `Host github.com-chengta100`，指向 `github.com` 并使用上面的专用 key。
- 不应该存在错误账号的本地凭据配置，例如 `credential.https://github.com.username 262933974`。

如果发现 remote 仍是 HTTPS，先切回专用 SSH 地址：

```bash
cd /Users/macbook/Documents/GitHub/-100
git remote set-url origin git@github.com-chengta100:hellarychou-cell/-100.git
ssh -T github.com-chengta100
```
...
根因不是代码构建失败，而是同步目标仓库残留了错误 GitHub 用户名配置，导致 `git push` 读取错误钥匙串凭据。当前稳定方案已经改为专用 SSH key，不再依赖 GitHub HTTPS 密码/token。下次遇到类似 GitHub 密码/凭据错误，先查 remote 是否被改回 HTTPS，不要反复重跑同步脚本。
...
3. 在 `/Users/macbook/Documents/GitHub/-100` 做上面的 Git/SSH 预检。
4. 回到 `/Volumes/PS2000/成她` 执行 `./scripts/sync-to-github.sh "更新说明"`。
5. 如果脚本已经 commit 但 push 失败，不要重新提交；进入 `/Users/macbook/Documents/GitHub/-100` 检查 `git remote -v` 和 `ssh -T github.com-chengta100` 后直接 `git push origin main`。
